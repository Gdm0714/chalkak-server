...
import com.min.chalkakserver.service.PhotoBoothReportService;
...
public class PhotoBoothController {
    ...
    private final PhotoBoothReportService photoBoothReportService;
    ...
    @GetMapping("/popular")
    @Operation(summary = "인기 네컷사진관 조회", description = "리뷰 평점과 리뷰 수를 기반으로 인기 네컷사진관을 조회합니다")
    public ResponseEntity<List<PhotoBoothResponseDto>> getPopularPhotoBooths(
            @Parameter(description = "조회할 개수 (최대 50)")
            @RequestParam(defaultValue = "20") @Min(1) @Max(50) int limit) {
        List<PhotoBoothResponseDto> popularBooths = photoBoothService.getPopularPhotoBooths(limit);
        return ResponseEntity.ok(popularBooths);
    }

    @GetMapping("/brands")
    @Operation(summary = "브랜드 목록 조회", description = "등록된 모든 네컷사진관 브랜드 목록을 조회합니다")
    public ResponseEntity<List<String>> getAllBrands() {
        List<String> brands = photoBoothService.getAllBrands();
        return ResponseEntity.ok(brands);
    }

    @PostMapping("/report")
    @Operation(summary = "네컷사진관 제보", description = "새로운 네컷사진관 정보를 제보합니다. 로그인 시 제보 이력을 추적할 수 있습니다.")
    public ResponseEntity<ReportResponseDto> reportPhotoBooth(
            @Valid @RequestBody PhotoBoothReportDto reportDto,
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        Long userId = userDetails != null ? userDetails.getId() : null;
        ReportResponseDto response = photoBoothReportService.submitReport(reportDto, userId);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/reports/my")
    @Operation(summary = "내 제보 목록", description = "내가 제보한 네컷사진관 목록과 처리 상태를 조회합니다")
    @SecurityRequirement(name = "bearerAuth")
    public ResponseEntity<List<ReportResponseDto>> getMyReports(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        List<ReportResponseDto> response = photoBoothReportService.getMyReports(userDetails.getId());
        return ResponseEntity.ok(response);
    }

    @GetMapping("/reports/my/stats")
    @Operation(summary = "내 제보 통계", description = "내 제보 통계 (총 제보 수, 승인된 제보 수)")
    @SecurityRequirement(name = "bearerAuth")
    public ResponseEntity<Map<String, Long>> getMyReportStats(
            @AuthenticationPrincipal CustomUserDetails userDetails) {
        Map<String, Long> stats = photoBoothReportService.getMyReportStats(userDetails.getId());
